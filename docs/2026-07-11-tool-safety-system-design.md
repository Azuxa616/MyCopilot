...
- **缓存 key**：`sessionId + agentId + toolRef + policyVersion + resourceScope`；`toolRef` 必须包含工具 ID 和来源，不能只使用名称。
- **资源范围**：文件类工具使用规范化、受根目录约束的路径前缀；HTTP 工具使用规范化 origin；无法安全计算范围的工具不缓存确认。
...
2. 无法定义资源范围的受限操作（例如带任意参数的第三方写操作）不复用确认，按每次确认处理。
3. 不同 agent、不同 MCP 来源、不同策略版本和不同参数摘要不得共享授权。
