/**
 * 插件清单与生命周期 —— TypeScript 类型。
 *
 * 自包含模块：不从 @my-copilot/shared 或任何其他包导入。每个类型都内联声明，
 * 使此文件可通过 `tsc --noEmit docs/rfc/types/plugin-manifest.d.ts` 独立完成类型检查。
 *
 * 这些类型镜像：
 *   - docs/rfc/schemas/plugin.manifest.schema.json
 *   - docs/rfc/schemas/plugin.lifecycle-event.schema.json
 *   - docs/rfc/plugin-manifest-lifecycle.md
 *
 * 它们是规范类型，不是运行时代码，因此不携带任何实现。
 */
...
/**
 * 插件标识符。小写 kebab-case，跨版本稳定，用作 `pluginId:resourceName`
 * 引用中的命名空间前缀。
 */
 ...
 * 插件的来源层级（决策 A4）。
 *
 * 注意：'built-in' 被有意省略 —— 内置项编译进宿主二进制，且从不走插件协议。
 * 只有官方（'official'）和用户安装（'community'）插件可通过此清单寻址。
 */
...
/**
 * 插件类型系统（决策 A1）。当前主要类型是 'frontend-response'。未来类型构成
 * 一个开放枚举，调用者必须将其视为前向兼容：未知值表示"将插件视为
 * 不透明但仍可安装"。
 */
...
/**
 * 文件系统权限作用域。所有路径都相对于 $PLUGINS_DIR/<plugin_id>/ 解释；
 * 空数组或缺失表示无该类访问。
 */
...
/** Context Provider 引用（MAY 扩展点，A3）。 */
export interface ContextProviderRef {
  id: string;
}

/** Memory Backend 引用（MAY 扩展点，A3）。 */
 ...
 * 插件清单（plugin.json）。
 *
 * 精确镜像 docs/rfc/schemas/plugin.manifest.schema.json。九个必需字段
 * （name、version、description、author、license、engineCompatibility、
 * source、permissions、provides）在此也是必填的。
 */
export interface PluginManifest {
  ...
  /** 可选的显式插件类型；默认为 'frontend-response'（A1）。 */
  type?: PluginType;
}
...
/**
 * PluginPerformanceBudget 的解析默认值。值由 RFC
 * （plugin-manifest-lifecycle.md，规范 §10）定义。
 */
export declare const DEFAULT_PLUGIN_BUDGET: PluginPerformanceBudget;
